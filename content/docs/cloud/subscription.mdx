---
title: Subscription
---

Umami uses **Stripe** as its payment processor. When signing up for a plan you will be sent to the Stripe checkout portal.
All subscriptions come with a 14-day free trial. You are free to cancel within that time frame at zero charge.

Details for each Umami Cloud plan can be found on the [pricing](https://umami.is/pricing) page.

## Upgrade to Pro

Once logged in, click on the side nav profile button then click **Settings**.

![image](/images/cloud/navbar-settings.png)

Navigate to **Billing** and click on the **Upgrade to Pro** button.

![image](/images/cloud/billing-plans.png)

If you are interested in an **Enterprise** plan, please click the **Contact Us** button or email us at sales@umami.is

You will be sent to the Stripe checkout portal to finish signing up. Navigate to **Billing** to see your new billing details.

![image](/images/cloud/billing-details.png)

## Estimated Cost & Spending Budget

Monthly **Pro** and **Business** plans show an estimated cost breakdown on the **Billing** page:
base plan price plus any event and session replay overage for the current period, excluding taxes.

Click **Manage budget** to set a monthly spending budget in USD. It can't go below your plan's base
price, since that's charged regardless of usage. If estimated cost exceeds the budget, Umami stops
recording events until the next billing period, or until you raise or remove the budget. You'll get
an email when this happens. Not available on annual plans, since those are prepaid.

## Manage Plan

Navigate to **Billing** and click on the **Manage plan** button. You will be sent to the Stripe billing portal where you can do the following.

- Cancel your **Pro** plan
- Update payment method
- Update billing information
- Review invoice history
- Download invoices and receipts

![image](/images/cloud/billing-manage-plan.png)
