...
1. Create an account on [Railway](https://railway.app/) connected to GitHub.
1. If you wish, you can give Railway permission only to your Umami fork repository when linking your GitHub account.
1. From the dashboard page click **New Project > Deploy from repo**.
1. Choose your forked Umami repository and select the **master** branch.
1. Add a first variable called `PORT`, set its value to a valid port number (eg: `3000`). Click **Add**.
1. Add a second variable called `DATABASE_TYPE`, set its value to `postgres`. Click **Add**.
1. Add a third variable called `HOSTNAME`, set its value to `0.0.0.0`. Click **Add**.
1. Click **Deploy**.
...
1. On the dashboard, select your Umami service, go to the **Variables** tab and click **New Variable**.
1. Click **Add Reference** and select the `DATABASE_URL` variable from your Postgres database. Click **Add**.
